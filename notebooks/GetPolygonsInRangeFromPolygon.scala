...
import h3xwrapper.utils.getH3EdgeLength
  ...
  def getPolygonsInRangeFromPolygons(polygonsDfSource: DataFrame
    ...
    val targetPolygonsTransformed: DataFrame = polygonsDfTarget.getGeometrySimplified(targetGeometryCol, distanceTolerance)
      .withColumn(geometry_centroid,ST_Centroid(targetGeometryCol))
...
val experimentId:String = "PLACES_IN_10KM_RANGE_FROM_BG"
def sedonaFun: DataFrame = sedonaGetGeometryInRangeFromPolygon(bgDataset,"bg_shape",places,"place_shape",10000)
def h3Fun(h3Resolution: Int): DataFrame = getPolygonsInRangeFromPolygons(
  bgDataset,"bg_shape",places,"place_shape",10000,h3Resolution
)
...
val res6qualityCheckResult = qualityCheck(spark)(basePath,experimentId,6,Seq("place_id","bg_id"))
...
val res7qualityCheckResult = qualityCheck(spark)(basePath,experimentId,7,Seq("place_id","bg_id"))
...
val overallResultTime = Seq(res5ResultAvgFinal,res6ResultAvgFinal,res7ResultAvgFinal).reduceLeft(_.unionByName(_)).join(sedonResultAvgFinal,Seq("id"))
val overallResultQuailty = Seq(res5qualityCheckResult,res6qualityCheckResult,res7qualityCheckResult).reduceLeft(_.unionByName(_))
